...
function ajax(url, method, args, callback)
{
  var argstring = "";
  for (var k in args)
    argstring += (argstring.length == 0 ? "?" : "&") + k + "=" + encodeURIComponent(args[k]);
  var xhr = new XMLHttpRequest();
  xhr.onreadystatechange = function () {
    xhr.readyState === 4 && xhr.status === 200 && callback && callback(JSON.parse(xhr.responseText));
  };
  xhr.open(method, url + argstring);
  xhr.send(null);
}
document.addEventListener("DOMContentLoaded", function() {
  ajax("/api/config", "GET", {}, function(r) {
...
<body class="container">
  <div class="row">
    ...
    <div><input type="color" id="color2" name="color2" value="#ffffff" /></div>
    <label for="fade_duration">Dauer zwischen Wechsel (Millisekunden):</label>
    <div><input type="number" min="0" max="4294967295" id="fade_duration" name="fade_duration" value="0" /></div>
    <div><button type="button" onclick="saveConfig()">Speichern</button></div>
  </div>
</body>
</html>)====="